...
import { Image as ImageIcon, UploadCloud, Trash2, X, Save, Clock, Phone, ThumbsUp, ThumbsDown } from 'lucide-react';
...
export default function AdminFollowUp({ onNavigate, onLogout }) {
  ...
  const [form, setForm] = useState({
    delayMinutes: 5, callNumber: '', callCtaText: 'Call Us',
    promptBody: '', interestedBody: '', notInterestedBody: '',
  });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(e?.response?.data?.error || e.message || 'Failed to load follow-up settings');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <AdminShell active="followup" onNavigate={onNavigate} onLogout={onLogout} title="Follow-up Automation">
      <div className="mb-6 animate-fade-in-up">
        <h2 className="text-[18px] font-bold text-slate-800 tracking-tight">Interested / Not Interested follow-up</h2>
        <p className="text-[14px] text-slate-500 font-medium mt-1 max-w-3xl">
          After a customer picks a service and receives the demo, we wait a few minutes and then send an
          Interested / Not Interested prompt. Based on their tap, they get a tailored reply with a Call button,
          and their status is recorded as Interested / Not Interested in the dashboard.
        </p>
      </div>
        ...
        <div className="space-y-6 animate-fade-in-up stagger-2 max-w-4xl">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 grid sm:grid-cols-3 gap-4">
            <Field label="Delay before prompt (minutes)" icon={Clock}>
              <input type="number" min="0" className="adm-input" value={form.delayMinutes}
                onChange={(e) => setForm((f) => ({ ...f, delayMinutes: e.target.value }))} />
            </Field>
            <Field label="Call number (Call CTA)" icon={Phone}>
              <input className="adm-input" value={form.callNumber}
                onChange={(e) => setForm((f) => ({ ...f, callNumber: e.target.value }))} placeholder="918106811285" />
            </Field>
            <Field label="Call button label">
              <input className="adm-input" maxLength={20} value={form.callCtaText}
                onChange={(e) => setForm((f) => ({ ...f, callCtaText: e.target.value }))} placeholder="Call Us" />
            </Field>
          </div>
          ...
          <Card title="Follow-up prompt (with Interested / Not Interested buttons)">
            <ImageRow
              slot="followup" url={cfg?.promptHeader} busy={busySlot === 'followup'}
              onPick={(f) => pickImage('followup', f)} onRemove={() => removeImage('followup')}
            />
            <textarea className="adm-input resize-none mt-3" rows={3} value={form.promptBody}
          ...
          <Card title="Interested reply" icon={ThumbsUp} accent="emerald">
            <ImageRow
              slot="interested" url={cfg?.interestedHeader} busy={busySlot === 'interested'}
              onPick={(f) => pickImage('interested', f)} onRemove={() => removeImage('interested')}
            />
            <textarea className="adm-input resize-none mt-3" rows={3} value={form.interestedBody}
              onChange={(e) => setForm((f) => ({ ...f, interestedBody: e.target.value }))}
          ...
          <Card title="Not Interested reply" icon={ThumbsDown} accent="amber">
            <ImageRow
              slot="not_interested" url={cfg?.notInterestedHeader} busy={busySlot === 'not_interested'}
              onPick={(f) => pickImage('not_interested', f)} onRemove={() => removeImage('not_interested')}
            />
            <textarea className="adm-input resize-none mt-3" rows={3} value={form.notInterestedBody}
              onChange={(e) => setForm((f) => ({ ...f, notInterestedBody: e.target.value }))}
...
function Card({ title, icon: Icon, accent = 'slate', children }) {
  const accentCls = accent === 'emerald' ? 'text-emerald-600' : accent === 'amber' ? 'text-amber-600' : 'text-admin-accent';
  return (
  ...
  return (
    <div className="flex items-center gap-4">
      <div className="w-28 h-20 bg-slate-100 rounded-xl overflow-hidden flex items-center justify-center border border-slate-200 shrink-0">
        {url ? <img src={url} alt="" className="w-full h-full object-cover" /> : <ImageIcon size={22} className="text-slate-300" />}
      </div>
      <div className="flex items-center gap-2">
        ...
        {url && (
          <button onClick={onRemove} disabled={busy} className="inline-flex items-center gap-1.5 px-3 py-2 text-[13px] font-medium text-rose-600 hover:bg-rose-50 rounded-xl disabled:opacity-60">
