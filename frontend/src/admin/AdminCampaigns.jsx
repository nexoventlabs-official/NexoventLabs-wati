...
export default function AdminCampaigns({ onNavigate, onLogout }) {
  ...
  useEffect(() => {
    ...
    return () => {
      socket.off("campaign:update", onUpd);
      socket.off("campaign:delete", onDel);
    };
  }, []);
  ...
  async function del(item) {
    if (!confirm(`Remove ${fmtPhone(item.waId)} from the campaign list?`))
      return;
    try {
  ...
  async function deleteSelected() {
    const ids = [...selected];
    if (!ids.length) {
      showToast("error", "Select at least one contact.");
      return;
    }
    if (
      !confirm(
        `Delete ${ids.length} selected contact(s) from the campaign list?`,
      )
    )
      return;
    setDeleting(true);
  ...
  async function handleSendOrSchedule({ mode, scheduledAt }) {
    ...
    try {
      if (mode === "now") {
        ...
        if (r.rateLimited)
          parts.push(
            `${r.rateLimited} queued for auto-retry (Meta marketing cap)`,
          );
        if (r.skipped)
          parts.push(`${r.skipped} skipped (already sent recently)`);
  ...
  const stats = useMemo(() => {
    ...
    for (const item of items)
      counts[item.lastStatus] = (counts[item.lastStatus] || 0) + 1;
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 mb-6 animate-fade-in-up">
        <div>
          ...
          <p className="text-[14px] text-slate-500 font-medium mt-1 max-w-2xl">
            Add WhatsApp numbers and send the approved welcome template to them.
            Duplicates are skipped automatically. Numbers that aren't on
            WhatsApp are flagged so you don't waste sends. Every contact also
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm animate-fade-in-up stagger-2">
        <div className="flex flex-wrap items-center gap-3 p-4 border-b border-slate-100">
          ...
          <div className="flex items-center gap-2 ml-auto flex-wrap">
            ...
            >
              <Send size={15} />{" "}
              {sending ? "Processing…" : `Send to selected (${selected.size})`}
            </button>
                  ...
                  <td colSpan={6} className="text-center py-16 text-slate-400">
                    <div className="flex flex-col items-center justify-center gap-2">
                      ...
                      <div className="text-[13px]">
                        Click "Add numbers" to start a campaign list.
                      </div>
                    </div>
                  </td>
                </tr>
                ...
                items.map((c) => {
                  ...
                  return (
                    <tr
                      key={c._id}
                      className={
                        isSel ? "bg-admin-accent/5" : "hover:bg-slate-50"
                      }
                    >
                      <td className="px-4 py-3">
                        <input
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[12px] font-semibold border ${st.cls}`}
                          title={c.lastError || ""}
                        >
                          {st.icon} {st.label}
                        </span>
                        ...
                        {c.lastStatus === "rate_limited" && (
                          <div className="text-[11px] text-orange-600 mt-1">
                            Meta marketing cap — auto-retrying
                          </div>
                        )}
                      </td>
                      <td className="px-4 py-3 text-slate-500 text-[13px]">
                        {c.lastStatus === "scheduled" && c.scheduledAt ? (
                          ...
                          <span className="text-orange-600 font-medium flex items-center gap-1">
                            <RefreshCw size={13} />
                            Retry after{" "}
                            {new Date(c.retryAfter).toLocaleString()}
                          </span>
                        ) : c.lastSentAt ? (
      ...
      {adding && (
        <AddModal
          ...
          onAdded={(r) => {
            ...
            showToast(
              "success",
              `Added ${r.added} number(s)${r.duplicates ? `, ${r.duplicates} duplicate(s) skipped` : ""}.`,
            );
          }}
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        <div className={`text-[20px] font-bold leading-none ${c.val}`}>
          {value}
        </div>
        <div className={`text-[11px] font-medium mt-0.5 truncate ${c.text}`}>
          {label}
        </div>
      </div>
    </div>
...
function SendOrScheduleModal({
  ...
  function validate() {
    ...
    if (!scheduledDT) {
      setDtError("Please pick a date and time.");
      return false;
    }
    ...
    if (isNaN(chosen.getTime())) {
      setDtError("Invalid date/time.");
      return false;
    }
    if (chosen <= now) {
      setDtError("Scheduled time must be in the future.");
      return false;
    }
    if (chosen.getTime() - now.getTime() < 60 * 1000) {
      setDtError("Schedule at least 1 minute ahead.");
      return false;
    }
    setDtError("");
    return true;
  ...
  function handleConfirm() {
    ...
    onConfirm({
      mode,
      scheduledAt:
        mode === "schedule" ? new Date(scheduledDT).toISOString() : null,
    });
  }
      ...
      >
        ...
        <div className="p-6 space-y-5">
          <p className="text-[14px] text-slate-600">
            You're about to send the welcome template to{" "}
            ...
            <div className="flex items-start gap-2.5 px-3 py-2.5 bg-amber-50 border border-amber-200 rounded-xl text-[13px] text-amber-800">
              ...
              <span>
                <span className="font-semibold">
                  {skippableCount} contact{skippableCount !== 1 ? "s" : ""}
                </span>{" "}
                were already sent to within the last 23 hours and will be
                skipped to prevent duplicate messages.{" "}
                {willSend > 0 ? (
                  <span className="font-semibold">
          ...
          {mode === "schedule" && (
            <div className="space-y-2">
              ...
              <p className="text-[12px] text-slate-400">
                Times are shown in your local timezone. Schedule must be at
                least 1 minute from now.
              </p>
            </div>
          )}
        </div>
...
function AddModal({ onClose, onAdded, onError }) {
  ...
  async function save() {
    ...
    if (!list.length) {
      onError("Enter at least one number.");
      return;
    }
    setBusy(true);
    try {
  ...
  return (
    ...
    >
      <div
        className="bg-white w-full max-w-lg rounded-2xl shadow-premium-hover"
        onMouseDown={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
        ...
        <div className="p-6 space-y-4">
          <div>
            <div className="text-[12px] uppercase tracking-wider font-semibold text-slate-500 mb-1.5">
              Name (optional)
            </div>
            <input
              className="adm-input"
              ...
              placeholder="Applies to all added in this batch"
            />
          </div>
          <div>
            <div className="text-[12px] uppercase tracking-wider font-semibold text-slate-500 mb-1.5">
            ...
            {queuedNumbers.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {queuedNumbers.map((n) => (
                  <div
                    key={n}
                    className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-100 text-slate-700 text-xs font-medium"
                  >
                    +{n}
                    <button
            ...
            <p className="text-[12px] text-slate-400 mt-2">
              Default country code is +{countryCode}. Click the + button to
              queue numbers.
            </p>
          </div>
        </div>
        <div className="flex items-center justify-end gap-2 px-6 py-4 border-t border-slate-100">
          ...
          <button
            onClick={save}
            disabled={busy}
            className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-admin-accent to-admin-accentHover text-white text-sm font-semibold shadow-md disabled:opacity-60"
          >
